...
namespace Hazel {
	...
	struct Renderer2DData {
		...
		uint32_t TextureSlotIndex; // 0 is reserved for white pixel texture
	};
	...
	void Renderer2D::DrawRotatedQuad(const glm::vec2& position, const glm::vec2& size, float rotation, Ref<Texture2D>& texture, float tilingFactor, const glm::vec4& tintColor) {
		Renderer2D::DrawRotatedQuad({ position.x, position.y, 0.0f }, size, rotation, texture, tilingFactor, tintColor);
	}
